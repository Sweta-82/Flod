...
const AnimatedList = ({
  ...
  return (
    <div className={`relative w-full ${className}`}>
      <div
        ref={listRef}
        className={`max-h-[520px] overflow-y-auto p-2 sm:p-4 ${
          displayScrollbar
            ? '[&::-webkit-scrollbar]:w-[8px] [&::-webkit-scrollbar-track]:bg-surface [&::-webkit-scrollbar-thumb]:bg-primary/40 [&::-webkit-scrollbar-thumb]:rounded-[4px]'
            : 'scrollbar-hide'
        }`}
        onScroll={handleScroll}
        style={{
          scrollbarWidth: displayScrollbar ? 'thin' : 'none',
          scrollbarColor: '#A0307A #120F17'
        }}
      >
        {items.map((item, index) => (
          ...
          >
            {renderItem ? (
              ...
              <div className={`p-4 bg-surface border border-border/80 rounded-2xl transition shadow-sm ${selectedIndex === index ? 'bg-primary/10 border-primary/40' : ''} ${itemClassName}`}>
                {typeof item === 'string' ? <p className="text-foreground font-medium m-0">{item}</p> : item}
              </div>
            )}
          </AnimatedItem>
        ))}
